...
nSample=100;
Water2PowerInterLinkSample=zeros(nPowerNodeGateStation,nCol-1,nSample);
Power2WaterInterLinkSample=zeros(nWaterNodePumpStation,nCol-1,nSample);
PowerAndWaterInterLinkSample=zeros(nWaterNodePumpStation+nPowerNodeGateStation,nCol-1,nSample);
nPowerAndWaterAllLinkSample=nWaterNodePumpStation+nPowerNodeGateStation+length(WaterPowerLinkTable.NodeStartId);
PowerAndWaterAllLinkSample=zeros(nPowerAndWaterAllLinkSample,nCol-1,nSample);
for iSample=1:nSample
    ...
    for jGateStation=1:nPowerNodeGateStation
        ...
        Water2PowerInterLinkSample(jGateStation,:,iSample)=PrWater2PowerTable{RowsSampleIndex,1:(nCol-1)};
    end
    ...
    for jPumpStation=1:nWaterNodePumpStation
        ...
        Power2WaterInterLinkSample(jPumpStation,:,iSample)=PrPower2WaterTable{RowsSampleIndex,1:(nCol-1)};
    end
    % concatenate arrays of links and interlinks
    PowerAndWaterInterLinkSample(:,:,iSample)=vertcat(Water2PowerInterLinkSample(:,:,iSample),...
        Power2WaterInterLinkSample(:,:,iSample));
    % combine interlinks and links of separate networks    
    PowerAndWaterAllLinkSample(:,:,iSample)=vertcat(WaterPowerLinkArray,Water2PowerInterLinkSample(:,:,iSample),...
        Power2WaterInterLinkSample(:,:,iSample));
...
ResiAllAfterDisruption=zeros(nSample,1);
...
%     AjacMat=adjacency(GraphAllInitial);
    ...
    WaterPowerLinkArray=PowerAndWaterAllLinkSample(:,:,iSample);
    ResiAllAfterDisruption(iSample,1)=calculate_resilience_after_disruption(WaterPowerNodeTableTemp, WaterPowerLinkArray);
end
